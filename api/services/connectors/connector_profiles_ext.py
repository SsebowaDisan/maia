...
Contains utility, community, research, and internal runtime connector profiles.
"""
...
PROFILES_EXT: dict[str, dict] = {
    ...
    "gmail_playwright": {
        ...
        "tools": [
            ToolSchema(id="gmail.send", name="Send email", description="Send an email via the Gmail browser interface.", action_class=ToolActionClass.execute, parameters=[
            ...
            ToolSchema(id="gmail.read", name="Read inbox", description="Read recent emails from the Gmail inbox.", action_class=ToolActionClass.read, parameters=[
    ...
    # ── B9 first-wave placeholders (metadata-only, no tools yet) ─────────────
    "notion": {"name": "Notion", "description": "Connect Notion workspaces for pages, databases, and task management.", "category": ConnectorCategory.data, "auth": NoAuthConfig(), "tags": ["notion", "docs", "project-management"]},
    "hubspot": {"name": "HubSpot", "description": "CRM contacts, deals, and marketing automation via HubSpot API.", "category": ConnectorCategory.crm, "auth": NoAuthConfig(), "tags": ["hubspot", "crm", "marketing"]},
    "salesforce": {"name": "Salesforce", "description": "CRM leads, opportunities, and account management via Salesforce API.", "category": ConnectorCategory.crm, "auth": NoAuthConfig(), "tags": ["salesforce", "crm"]},
    "jira": {"name": "Jira", "description": "Issue tracking, sprint management, and project boards via Jira Cloud API.", "category": ConnectorCategory.project_management, "auth": NoAuthConfig(), "tags": ["jira", "project-management", "atlassian"]},
    "airtable": {"name": "Airtable", "description": "Spreadsheet-database hybrid for structured data management.", "category": ConnectorCategory.data, "auth": NoAuthConfig(), "tags": ["airtable", "database", "spreadsheet"]},
    "zendesk": {"name": "Zendesk", "description": "Customer support tickets, knowledge base, and agent workflows.", "category": ConnectorCategory.support, "auth": NoAuthConfig(), "tags": ["zendesk", "support", "helpdesk"]},
    "stripe": {"name": "Stripe", "description": "Payments, subscriptions, and financial reporting via Stripe API.", "category": ConnectorCategory.commerce, "auth": NoAuthConfig(), "tags": ["stripe", "payments", "commerce"]},
    "shopify": {"name": "Shopify", "description": "E-commerce orders, products, and inventory via Shopify API.", "category": ConnectorCategory.commerce, "auth": NoAuthConfig(), "tags": ["shopify", "ecommerce", "commerce"]},
}
