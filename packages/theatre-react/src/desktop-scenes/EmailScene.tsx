...
function splitLeadingHeading(text: string): { heading: string; body: string } | null {
  ...
  if (normalized.length < MIN_PARAGRAPH_CHARS_FOR_SPLIT) return null;
  const match = normalized.match(/^(.{4,64}?)\s{1,3}([A-Z][\s\S]{32,})$/);
  if (!match) return null;
  const heading = String(match[1] || "").trim();
  const body = String(match[2] || "").trim();
  if (!heading || !body || /[.!?;:]$/.test(heading) || !isTitleCaseHeadingCandidate(heading)) {
    return null;
  }
  return { heading, body };
}
...
function focusedEmailField({
  ...
  if (normalizedType === "email_set_to") return "to";
  if (normalizedType === "email_set_subject") return "subject";
  if (normalizedType === "email_set_body" || normalizedType === "email_type_body") return "body";
  if (String(action || "").trim().toLowerCase() !== "type") return null;

  const label = String(actionTargetLabel || "").trim().toLowerCase();
  if (!label) return "body";
  if (label.includes("subject")) return "subject";
  if (label.includes("recipient") || label.includes("email") || label.includes("to")) return "to";
  if (label.includes("message") || label.includes("body") || label.includes("content")) return "body";
  return "body";
}
...
function EmailScene({
  ...
  const focus = focusedEmailField({ eventType: activeEventType, action, actionTargetLabel });
  ...
  const focusPulse = isFieldUpdateEvent || (action === "type" && (actionPhase === "start" || actionPhase === "active"));
    ...
    <div className="absolute inset-0 bg-[radial-gradient(circle_at_12%_8%,rgba(168,216,255,0.92),rgba(122,176,244,0.72)_40%,rgba(98,148,232,0.9)_100%)] p-9 text-[#1d1d1f]">
      <div className="mx-auto flex h-full w-full max-w-[840px] flex-col overflow-hidden rounded-[20px] border border-black/[0.1] bg-[#fcfcfd] shadow-[0_26px_58px_-42px_rgba(0,0,0,0.52)]">
        ...
        <div className="relative flex min-h-0 flex-1 flex-col gap-3 overflow-hidden p-5 pt-12 text-[12px]">
          ...
          />
          <div className={`grid grid-cols-[70px_minmax(0,1fr)] items-center rounded-[14px] border px-4 py-2.5 transition-all duration-300 ${
            focus === "to" && focusPulse ? "border-black/25 bg-[#f2f2f4]" : "border-black/[0.07] bg-[#fafafc]"
          }`}>
            <span className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[#8b8b91]">To</span>
            <span className="truncate text-[13px] text-[#1d1d1f]">{emailRecipient || "recipient@example.com"}</span>
          </div>
          <div className={`grid grid-cols-[70px_minmax(0,1fr)] items-center rounded-[14px] border px-4 py-2.5 transition-all duration-300 ${
            focus === "subject" && focusPulse ? "border-black/25 bg-[#f2f2f4]" : "border-black/[0.07] bg-[#fafafc]"
          }`}>
            <span className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[#8b8b91]">Subject</span>
            <span className="truncate text-[13px] font-medium text-[#1d1d1f]">{emailSubject || "No subject"}</span>
          </div>
          <div className={`flex min-h-0 flex-1 flex-col overflow-hidden rounded-[16px] border transition-all duration-300 ${
            focus === "body" && focusPulse ? "border-black/25 bg-[#f9f9fa]" : "border-black/[0.07] bg-white"
          }`}>
