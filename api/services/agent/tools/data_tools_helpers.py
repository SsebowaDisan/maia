    ...
    if isinstance(ga4_full_report, dict) and ga4_full_report:
        ...
        if pages_rows:
            ...
            for row in pages_rows[:10]:
                ...
                bounce = round(float(row.get("bounceRate") or 0) * 100, 1)
                lines.append(
                    f"| {page[:80]} | {views:,} | {minutes}m {seconds:02d}s | {bounce}% |"
                )
...
def _analytics_insight_highlights(settings: dict[str, Any]) -> list[str]:
    ...
    highlights = [
        (
            f"Traffic baseline: {sessions:,} sessions, {users:,} users, and {conversions:,} conversions "
...
def _analytics_insight_paragraphs(settings: dict[str, Any]) -> list[str]:
    ...
    paragraph_one = (
        "Performance summary: the latest 30-day window captured "
        f"{sessions:,} sessions, {users:,} users, and {conversions:,} conversions. "
