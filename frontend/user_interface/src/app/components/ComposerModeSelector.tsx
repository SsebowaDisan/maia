import { Bot, ChevronDown, Globe, Search, Sparkles } from "lucide-react";
...
export function ComposerModeSelector({ value, onChange }: ComposerModeSelectorProps) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        align="start"
        sideOffset={8}
        className="w-[190px] rounded-2xl border-black/[0.08] bg-white p-1.5 shadow-[0_20px_34px_-24px_rgba(0,0,0,0.55)]"
      >
        <div className="space-y-1">
          {MODE_OPTIONS.map((option) => (
            ...
            >
              ...
              {value === option.value ? <span className="text-[10px]">Active</span> : null}
            </button>
          ))}
        </div>
      </PopoverContent>
